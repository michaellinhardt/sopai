...
description: Optimize AI documents and prompts to reduce token consumption while preserving content authenticity. Use when compressing prompts, optimizing token count, reducing prompt size, or improving prompt efficiency. Accepts file paths or direct string content.
---
...
This skill provides methodology for semantic optimization and prompt compression for agentic AI systems. Apply analytical, precise, surgical approach. Methodical execution prioritizing original intent preservation.
...
This skill accepts two input modes:

1. **File Path Mode**: Provide a file path or folder path. Read and compress the content, overwriting the original file(s).
2. **Direct Content Mode**: Provide string content directly. Compress the content and write the output to a new file (specify output path or default to `compressed-output.md` in current directory).

**Usage Examples:**
- File path: `/path/to/prompt.md`
- Folder path: `/path/to/prompts/`
- Direct content: `"Your prompt text here..."` with optional output path
...
- **Mandatory Dependency:** Compression methodology dictated *exclusively* by annex file "Annex: Prompt Compression Protocol" below.
- **Modularity:** Never memorize/internalize protocol. Consult and apply step-by-step per request. Allows protocol updates without skill modification.
...
- **Must strictly** follow "Prompt Compression Protocol" annex as sole methodology source.
